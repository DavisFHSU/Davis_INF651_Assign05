...
function addTax(arr1) { 
	...
	let tax = 0.0925 * arr1[1];
	let newArray = [arr1[0],tax.toFixed(2),arr1[1],arr1[2]];
	return newArray;
}
...
function isPalindrome(string1) {
		let newString = string1.replace(/ /g, "");
		newString = newString.toLowerCase();
		let testFlag = true;
		for (i=0; i<newString.length; i++) {
			if (newString.charAt(i) != newString.charAt(newString.length-i-1)) testFlag = false;
